...
const getInitialTiles = () => {
    ...
    board.forEach((row, rowIndex) => (row.forEach((column, columnIndex) => {
        tiles.push({
            ...
            row: rowIndex,
            column: columnIndex,
            src: "/battleship/assets/framed-water.jpg",
            usedByShip: false,
            ship: undefined
        })
    })))
...
export default function SetUpGameBoard({ships, setShips}) {
    ...
    const markTiles = (canBeLaid, length, isHorizontal, overRow, overColumn) => {
        ...
        setTiles(tiles.map((e) => {
            ...
            return match ? {
                ...e,
                src: canBeLaid ? "/battleship/assets/green-framed-water.jpg" : "/battleship/assets/red-framed-water.jpg"
            } : {...e, src: "/battleship/assets/framed-water.jpg"}
        }))
    }

    const resetTileImages = () => {
        setTiles(tiles.map((e) => {
            return {...e, src: "/battleship/assets/framed-water.jpg"}
        }))
    }
